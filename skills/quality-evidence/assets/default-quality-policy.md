...
> where **risk is highest** and cheaper proofs structurally can't reach.
...
2. **Risk sets a floor; budget flexes only the ceiling.** Budget decides how far
   *up* the ladder you climb and how much redundancy you add. It can never take
   you below the floor (below). "Budget didn't allow it" is not a valid reason to
   skip a floor item.
3. **Defense-in-depth at the top.** For the highest-risk checks, deliberately
   stack layers (unit + integration + an agent/e2e gate) rather than picking the
   single cheapest sufficient proof — the cost of being wrong dwarfs the test
   cost.
...
- **Unit coverage on core and changed logic.** It's cheap; high coverage is
  expected.
- **At least one gate on every release-critical (weight-5) check.** Critical
  behavior always has *some* proof.
...
| Weight | Meaning | Default posture |
| --- | --- | --- |
| 5 | release-critical (billing, auth, data isolation, destructive) | DIRECT proof at a modality matching the check's nature + a gate; consider defense-in-depth |
| 4 | high | at least one DIRECT proof |
| 3 | important | DIRECT or strong INDIRECT proof |
| 1–2 | low | INDIRECT/STATIC acceptable; absence is a noted gap, not a push target |
