...
description: "Author, scaffold, and run coding-agent-driven tests: Markdown case files executed by a coding agent against a live environment (browser, API, DB, logs, cloud, telemetry) with an auditable PASS/FAIL/BLOCKED report. Use when a deterministic test would be premature, brittle, too expensive, or too narrow. Sibling to create-tests (YAML E2E); quality-evidence records the resulting evidence."
user_invocable: true
---
...
against a target environment and returns an auditable report. This is the
agent-driven counterpart to `create-tests`, which produces deterministic YAML
E2E tests run by Playwright plus the Shiplight agentic SDK.

| Skill | Test kind | Execution | Verification |
| --- | --- | --- | --- |
| `create-tests` | YAML E2E | Playwright + agentic SDK | Deterministic assertions |
| `create-agent-tests` | Markdown case | A coding agent (codex/claude/gemini) | Agent judgment + collected evidence |

This skill **authors and runs** agent tests. It does not own quality mapping:
once a case produces a report, hand the report path, final status, and evidence
artifacts to `quality-evidence`, which records them in `quality-map.yaml` and the
test report.
...
high-value stable agent flows into `create-tests` YAML E2E (or another standard
format) once the path stabilizes.
...
## Handoff To Quality Evidence

This skill produces; `quality-evidence` records. After a run, give
`quality-evidence` the report path, the final `PASS`/`FAIL`/`BLOCKED`/`ABORTED`
status, and the evidence artifacts so it can map them into
`quality-evidence/<target>/quality-map.yaml` and
`quality-evidence/<target>/test-report.md`. Do not duplicate quality mapping in
this skill.

## When Not To Use

- When a deterministic unit, contract, integration, or YAML E2E test is the
  cheaper, more durable proof — use `create-tests` or the standard format.
- When the user only wants quality assessment or a confidence report — use
  `quality-evidence`.
